...
def parse_ftrd_lines(log_path: str) -> list[dict]:
    ...
    try:
        ...
        for ftrd_info in ftrd_lines_info:
            ...
            ctx_lines = stdout2.read().decode("utf-8", errors="replace").strip().splitlines()
            client2.close()

            if ctx_lines:
                # Use fill price to find closest mid — handles multiple symbols firing simultaneously
                fill_price_raw = int(m.group(9))  # in paise
                best_line = None
                best_diff = float("inf")
                for cl in ctx_lines:
                    tm2 = MID_RE.search(cl)
                    if tm2:
                        mid_raw = int(tm2.group(1))
                        diff = abs(mid_raw - fill_price_raw)
                        if diff < best_diff:
                            best_diff = diff
                            best_line = cl
                if best_line:
                    tm = MID_RE.search(best_line)
                    if tm:
                        mid_val = int(tm.group(1)) / PRICE_DIVISOR
                        # match seen entry by token+fillnumber+fill_minute
                        ts_m2 = FTRD_IST_RE.search(ftrd_info)
                        fill_minute2 = ""
                        if ts_m2:
                            try:
                                ft2 = datetime.strptime(ts_m2.group(1), "%Y-%m-%d %H:%M:%S")
                                fill_minute2 = ft2.strftime("%Y%m%d%H%M")
                            except Exception:
                                pass
                        key = (token, fillnumber, fill_minute2)
                        if key in seen:
